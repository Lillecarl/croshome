...
//
// A rename must reach the hub long before that sweep: renaming a session
// right before agents address each other is the normal flow, and a stale
// title in `who` sends the message to a name nobody asked for. So
// session.updated is answered on the spot, gated on the title actually
// having changed -- the event fires for every metadata touch, and a hello
// per touch would be a process spawn per keystroke of session activity.
...
export const OcahubStopHook: Plugin = async ({ client, $, directory, worktree }) => {
  ...
  // What the hub was last told this session's title is. The gate of the
  // instant re-hello: a session.updated whose title matches this is
  // nothing the hub needs to hear about.
    ...
    event: async ({ event }) => {
      try {
        ...
        if (event.type === "session.updated") {
          ...
          // The event carries the fresh session info; only a title
          // change is news to the hub.
          const info: any = (event.properties as any)?.info
          const fresh = typeof info?.title === "string" ? info.title : undefined
          if (fresh === undefined || titles.get(sid) === fresh) return
          await helloSession(sid)
          return
        }

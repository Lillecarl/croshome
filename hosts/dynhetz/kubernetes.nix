...
in
{
  config = {
    ...
    # `kubeadm init`, run once and then never again.
    #
    # /var/lib/kubelet/config.yaml is the marker. kubeadm writes it near the end
    # of a successful init and nothing else on this host writes it, so its
    # presence means "this machine is already a node". The same file is what
    # gates kubelet below, which is why the two agree without talking.
    #
    # This unit is deliberately not idempotent in the Nix sense. Changing
    # ${"$"}{kubeadmConfig} does not re-provision the cluster; it changes what a
    # *future* init would do. Reprovisioning is `kubeadm reset` followed by a
    # restart of this unit, and it is a destructive operation that belongs in a
    # human's hands rather than in an activation script.
